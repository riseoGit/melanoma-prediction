...
from keras.applications.inception_resnet_v2 import BinaryInceptionResNet
from .balanced_generator import _balanced_batch_apply

class BalanceBinaryInceptionResnet(object):
    def __init__(self,
                 type,
                 retrain=False,
                 epochs = 5,
        ...
        self.block_count_a = block_count_a
        self.block_count_b = block_count_b
        self.block_count_c = block_count_c
    ...
    def init_model (self):
        print("binary_inception_resnet.init_model->type", self.type)
        print("binary_inception_resnet.init_model->retrain", self.retrain)
        ...
        optimizer = Adam(
            ...
            decay=0.0, 
            amsgrad=False)
        print("binary_inception_resnet.init_model->loss=", lossfunc)
        model.compile(
            loss=lossfunc,
            optimizer=optimizer,
            metrics=metrics
        ...
        print("binary_inception_resnet.init_model->end")
    def train(self, x_train, y_train, x_val, y_val):
        print("binary_inception_resnet.train->start")
        
        val_batch_size = self.val_batch_size
        if val_batch_size <= 0:
            val_batch_size = x_val.shape[0]
        if self.balance :
            print("binary_inception_resnet.train->balance")
            ...
            print("binary_inception_resnet.train->normal")
        ...
        print("binary_inception_resnet.train->filepath=",filepath)
        ...
        if self.epochs > 10:
            print("binary_inception_resnet.train->fit clr_fast")
        ...
        print("binary_inception_resnet.train->fit clr_slow ")
        ...
        print("binary_inception_resnet.train->end")
    def save (self, output_dir = "", fname = None):
        if output_dir == "":
            output_dir = self.output_dir
        if fname is None:
            fname = self._get_prefix()
        print("binary_inception_resnet.save->fname=",fname)   
        ...
        with open(hf, mode='w') as f:
            hist_df.to_json(f)
        print("binary_inception_resnet.save->end,fname=",fname) 
